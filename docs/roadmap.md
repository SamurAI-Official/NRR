...
## Verified status (as of M1, in progress)
...
| CPU inference | real ONNX execution | Real: `runtime/backend_cpu.cpp` feeds color/depth/motion tensors through an `OrtSession` and returns an RGB8 texture. 77/77 suite tests pass with the ORT SDK present. |
...
| Temporal rendering | Phase 4 complete | **Real but incomplete value**: the render path now drives history, motion reprojection and motion-adaptive alpha, blends the reprojected previous frame into the output image, and reports measured state (`runtime/backend_cpu.cpp`, verified by `tests/integration/test_temporal_accumulation.cpp`). What it accumulates is the untrained fixture, so flicker drops without detail improving; no disocclusion rejection, and the flicker gate is asserted per test but not against a committed baseline. |
...
- [x] **Temporal integration** (`M1.1`, done): `BackendCPU::execute_model` computes the
      frame's temporal state, reprojects the previous displayed frame through the current
      frame's motion field, blends it into the output image, records the displayed frame
      for the next iteration, and reports measured state plus a measured
      `temporal_stability`. The `output.temporal = input.temporal` passthrough is gone.
      Evidence (unified suite):
      * `test_temporal_state_reported_from_pipeline` - history depth `0/1/2`, alpha
        `0/0.7/0.3` for motion `0.0/0.0/0.7` (the third frame sits 0.4 above the
        threshold, where the decay rule gives `0.7 * (1 - 0.4/0.7) = 0.3`), i.e.
        availability and the decay rule are visible in the frame output rather than
        echoed from the input. The test feeds deliberately wrong alpha/history values
        in and requires the pipeline to overwrite them.
      * `test_temporal_accumulation_applies_measured_blend` - an accumulation run at low
        motion differs from a matched un-accumulated render by exactly the blend equation
        `(1 - alpha) * current + alpha * warped`: mean |error| 0.25 byte, worst 0.5 byte
        (anti-aliasing quantisation) across 8x16x3 output channels.
      * `test_temporal_motion_above_threshold_bypasses_history` - at motion 1.0 the
        accumulated render is the raw model output (mean delta < 1/255) with alpha 0, so
        history cannot ghost into a fast frame; the same scene change at motion 0.0 is
        measured moving away from the raw output by `alpha * raw change`.
      * `test_temporal_stability_reported_from_displayed_frames` - the expectation is
        derived from the downloaded pixels themselves, not hard-coded: a frame whose
        measured change is 0.2 reports stability 20 (observed `frame change: 0.2 ->
        stability 20 (expected 20)`), an identical consecutive frame reports 100, and a
        frame with no predecessor reports 100.
...
### M1.1 postmortem: tests that could not fail

Bringing the temporal path up exposed three tests that asserted nothing about the runtime:

1. `tests/integration/test_multi_frame.cpp` declared its **own** `nrr::test::TemporalHistory`
   duplicate and tested that, so the shipped `nrr::TemporalHistory` had no test at all.
   The duplicate is gone; the file now tests the real class.
2. `test_temporal_state_update` set `output.temporal.temporal_alpha = 0.7f` by hand and
   asserted it was `> 0` - a tautology. Renamed to
   `test_temporal_state_not_fabricated_without_model` and inverted: a render that cannot
   run must **not** rewrite the caller's frame state.
3. `tests/main.cpp` hand-listed 8 of the 16 latency tests, so `latency_motion_magnitude_alpha`
   and `latency_frame_index_continuity` never ran; it also registered a
   `test_android_thermal_states` that does not exist (a compile error on any non-Windows
   build) and never registered `test_ios_low_power_mode`. It now calls the latency
   aggregator and registers the tests that exist under the guards that apply.
4. `tests/mobile/test_mobile_model.cpp` defines six tests
   ...
   `test_mobile_buffer_operations`) and **none of them was registered**, while
   `CMakeLists.txt` documented the file as "integrated into the unified test suite via
   tests/main.cpp and run from nrr_tests.exe". The claim was true of the file - it is
   compiled in and has no `main()` - and false of the tests. They are now registered under
   the same `NRR_ENABLE_MOBILE_VENDOR` guard as the vendor tests. To be exact about what
   that fixes: the guard is off for desktop builds, so they still do not run in CI, and
   this repository has never built or executed the mobile targets, so these six tests have
   never run on any platform. What changed is that the gap is now explicit (the guard and
   the reason are stated in tests/main.cpp) instead of the file appearing covered.

Two production defects were also found and fixed while wiring this up:
`nrr::TemporalHistory::has_previous_frame()` ignored the frame index (it could report a
previous frame for a frame index that `get_previous_frame()` would refuse), and
`TemporalStateManager::update_state()` recorded the frame *before* its caller could see
the state, which is why the state and the recorded history could not be reconciled - it is
now split into `compute_state()` and `record_frame()`.

**Still a passthrough:** the ONNX path reports measured state, but the *placeholder* path
(`BackendCPU::fill_placeholder_stats()`, taken when no executable ONNX session exists)
still assigns `output.temporal = input.temporal` and reports a hard-coded
